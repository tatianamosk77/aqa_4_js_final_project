...
test.describe("[Smoke][Sales Portal][Orders] Manager", () => {
  ...
  test(
    "Should assign and then update manager",
    ...
    async ({
      ...
      const firstManager = await assignManagerModal.chooseDifferentManager();
      await assignManagerModal.clickSave();

      await orderDetailsPage.verifyToastMessage(NOTIFICATIONS.MANAGER_ASSIGNED);
      expect(await orderDetailsPage.getAssignedManagerName()).toBe(firstManager);

      await orderDetailsPage.editManager();
      const secondManager = await assignManagerModal.chooseDifferentManager();
      await assignManagerModal.clickSave();

      await orderDetailsPage.verifyToastMessage(NOTIFICATIONS.MANAGER_ASSIGNED);

      const finalName = await orderDetailsPage.getAssignedManagerName(); 
      expect(finalName).toBe(secondManager); 
