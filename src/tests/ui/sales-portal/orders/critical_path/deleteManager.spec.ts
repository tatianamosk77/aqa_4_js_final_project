...
test.describe("[Smoke][Sales Portal][Orders] Manager", () => {
  ...
  test(
    "Should assign and then remove manager",
    ...
    async ({
      ...
      expect(await orderDetailsPage.getAssignedManagerName()).toBe(managerName);
      
      await orderDetailsPage.removeManagerWithConfirmation();
      
      await orderDetailsPage.verifyToastMessage(NOTIFICATIONS.MANAGER_UNASSIGNED);
            
      await expect(orderDetailsPage.selectManagerLink).toBeVisible();
      await expect(orderDetailsPage.assignedManagerLink).not.toBeVisible();
    }
  );
});