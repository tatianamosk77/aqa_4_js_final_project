...
test.describe("[Smoke][Sales Portal][Orders] Manager", () => {
  ...
  test(
    "Should not remove manager when cancellation is cancelled in modal",
    ...
    async ({
      ...
      const response = await addNewOrderUIService.createMinimalOrderStable(customerName, productName);
      ...
      await orderDetailsPage.cancelManagerUnassignment();

      //
      
      const currentManager = await orderDetailsPage.getAssignedManagerName();
      expect(currentManager).toBe(managerName);
      
      await expect(orderDetailsPage.selectManagerLink).not.toBeVisible();
      
      const unassignToast = orderDetailsPage.getToastByText(NOTIFICATIONS.MANAGER_UNASSIGNED);
      await expect(unassignToast).not.toBeVisible();
    }
  );
});