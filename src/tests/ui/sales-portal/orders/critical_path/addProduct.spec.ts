...
test.describe("[UI][Orders][Edit Product details]", () => {
    let orderId: string;
    ...
    test("Should successfully Add product to a new order", 
        ...
        const addedProduct = await editProductsModal.addProductAndSelectUnique();
        ...
        await expect(orderDetailsPage.productNames).toContainText([addedProduct]);
    });
});